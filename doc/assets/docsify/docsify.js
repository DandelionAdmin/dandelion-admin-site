...
// no need for block level renderers
            ...
            function resolveUrl(base, href) {
                if (!baseUrls[' ' + base]) {
                    // we can ignore everything in base after the last slash of its path modules,
...
********************************************** */
        ...
        /* **********************************************
     Begin prism-structure-highlight.js
********************************************** */
        ...
        (function () {
            ...
            self.Prism.fileHighlight = function () {
                    ...
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState == 4) {
                            ...
                            if (xhr.status < 400 && xhr.responseText) {
                                ...
                                code.textContent = '✖ Error ' + xhr.status + ' while fetching structure: ' + xhr.statusText;
                            } else {
                ...
                if (Prism.plugins.toolbar) {
                    Prism.plugins.toolbar.registerButton('download-structure', function (env) {
...
// See https://github.com/PrismJS/prism/pull/1367
    ...
    Compiler.prototype._initRenderer = function _initRenderer() {
        ...
        origin.link = renderer.link = function (href, title, text) {
            ...
            if (
                ...
                // TODO EasyWeb 解决超链接href是javascript:;仍然有target="_blank"的bug
                // attrs += href.indexOf('mailto:') === 0 ? '' : (" target=\"" + linkTarget + "\"");
                if (!(href.indexOf('mailto:') === 0 || href.indexOf('javascript:') === 0)) {
    ...
    Compiler.prototype.sidebar = function sidebar(text, level) {
        ...
        if (text) {
            ...
            // TODO EasyWeb 解决sidebar中静态的多级菜单不能折叠的问题
            var fIndex = html.indexOf('<ul');
            html = html.substring(0, fIndex) + '<ul' + html.substring(fIndex + 3).replace(/<ul/g, '<ul class="app-sub-sidebar"').replace(/<li/g, '<li class="collapse"');
        } else {
    ...
    function highlight(path) {
        ...
        // TODO EasyWeb 解决侧边栏选中有偏差的问题
        // var top = ((doc && doc.scrollTop) || document.body.scrollTop) - coverHeight;
        var top = ((doc && doc.scrollTop) || document.body.scrollTop) - coverHeight + 30;
        var last;
    ...
    function renderMixin(proto) {
        ...
        proto._renderSidebar = function (text) {
            ...
            // TODO EasyWeb 解决动态的多级菜单展开不能折叠静态多级菜单的问题
            var _as = document.querySelectorAll('.sidebar-nav li.active>a');
            for (var i = 0; i < _as.length; i++) {
                var _lis = parentsUntil(_as[i], '.sidebar-nav', 'li.collapse');
                for (var j = 0; j < _lis.length; j++) {
                    _lis[j].classList.remove('collapse');
                }
            }
        };
